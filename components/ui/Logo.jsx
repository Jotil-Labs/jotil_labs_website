...
/**
 * Shared hex container paths used by all Jotil logos.
 * Two overlapping hex shapes: medium blue (#3B7BF2) and dark blue (#1B4FBA).
 */
export function HexContainer() {
  return (
    <>
      {/* Medium blue hex paths */}
      <g fill="#3B7BF2">
...
/**
 * JotilLabs hex logo with 3 ascending heartbeat dots.
 */
export default function Logo({ size = 'md', className }) {
  ...
  return (
    <svg
      ...
      aria-label="Jotil Labs logo"
      role="img"
    >
      <HexContainer />

      {/* Accent: 3 ascending heartbeat dots */}
      <path
      ...
      <path
        ...
        fill="#3B7BF2"
        style={{
          animation: 'hbDot 10s ease-in-out infinite 0.15s',
          transformBox: 'fill-box',
          transformOrigin: 'center',
        }}
      />
      <path
        ...
        fill="#3B7BF2"
        style={{
          animation: 'hbDot 10s ease-in-out infinite 0.3s',
          transformBox: 'fill-box',
          transformOrigin: 'center',
        }}
      />
...
/**
 * "Jotil Labs" text wordmark.
 * "Jotil" in dark text, "Labs" in primary blue.
 */
export function LogoText({ className }) {
  return (
    <span
      className={cn('font-sans font-[800] tracking-tight', className)}
...
/**
 * Combined hex logo + "Jotil Labs" text, side by side.
 */
export function LogoWithText({ size = 'md', className, textClassName }) {
  const dimension = typeof size === 'number' ? size : sizeMap[size] || sizeMap.md

  // Scale text relative to the icon size
  const textSize = dimension >= 56 ? 'text-2xl' : dimension >= 40 ? 'text-xl' : 'text-lg'

  return (
    <span className={cn('inline-flex items-center gap-2', className)}>
      <Logo size={size} />
      <LogoText className={cn(textSize, textClassName)} />
    </span>
  )
}
